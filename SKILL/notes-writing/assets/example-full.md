...
1. **方案 A（推荐，随数据仓存）**——图片放 Obsidian 风格附件目录 `content/<本文标题>.assets/`，写相对路径，构建期自动重写为 raw 绝对 URL：
   ...
   ![架构图](<在这里写文章标题.assets/arch-flow.webp>) <!-- 图片放 content/在这里写文章标题.assets/arch-flow.webp（与 .md 同名目录）。路径含空格/特殊字符时务必用尖括号包裹，否则不会被解析成图片 -->

   > ⚠️ **标题含空格时**：如 `在这里写文章标题.assets/...` 含空格，相对路径必须写成 `<在这里写文章标题.assets/x.webp>`（尖括号包裹），或把空格编码为 `%20`（等价写法 `在这里写文章标题.assets/x.webp` → `在这里写文章标题%20.assets/x.webp`）。裸写 `![alt](标题 含空格.assets/x.webp)` 会被当成纯文本、不显示。
