...
| 图片 | `![alt](<文章>.assets/<语义名>.webp)` | **方案 A**：图片随数据仓提交，写**相对路径**（`content/<文章>.assets/` 下，相对 `.md` 所在目录），构建期 `build.mjs` 自动重写为 `raw.githubusercontent.com/<owner>/<repo>/main/<仓库内相对路径>` 绝对 URL，站点零配置显示。也支持 `http(s)://` 完整外链、`data:` 内联、`/` 站点根（这三类不重写、不校验）。详见 §3.1 |
| 分割线 | `---` | |
...
正文在**站点域**渲染，Markdown 里的相对路径会被浏览器按站点根解析 → 404。因此图片采用**方案 A**：图片随数据仓提交，构建期由 `scripts/build.mjs` 把相对路径重写为 `raw.githubusercontent.com/<owner>/<repo>/main/<仓库内相对路径>` 的绝对 URL，站点零配置直接显示。
...
| 引用写法 | `![alt](<示例文章.assets/arch-flow.webp>)`（**尖括号包裹**，相对 `.md` 所在目录） |
| 不重写的三类 | ① `http(s)://` 完整外链 ② `data:` 内联 ③ `/` 开头（视为站点根，由站点自己提供） |
| 缺失文件 | 相对路径找不到文件 → 构建告警并保留原样（不会 404 到 raw，但会显示裂图） |
| 校验 | `npm run validate` 会拦截「相对路径没被重写」的图（`http(s):` / `data:` 除外），构建/校验失败要修到通过 |

> ⚠️ 本仓为**公开仓库**：图片会随 `content/<文章>.assets/` 一并公开，且经 `raw.githubusercontent.com` 直链。请勿放涉密/隐私图；大图建议走对象存储后用完整外链。

> ⚠️ **标题含空格时路径必须包裹尖括号**：`<文章>.assets/` 的 `<文章>` 取自文件名，若标题含空格（如 `Markdown 全功能示例`），裸写 `![alt](Markdown 全功能示例.assets/x.webp)` 会被 Markdown 解析器当成纯文本、**不显示**。正确写法二选一：① 尖括号包裹 `![alt](<Markdown 全功能示例.assets/x.webp>)`（推荐，模板即此写法）；② 空格编码 `%20`：`Markdown%20全功能示例.assets/x.webp`。构建脚本对两类都按 raw 绝对 URL 重写。
...
8. **图片随数据仓公开**：`content/<文章>.assets/` 下的图会经 `raw.githubusercontent.com` 直链公开，**勿放涉密/隐私图**；大图（>200KB）走对象存储后用完整外链，别堆在仓里。相对路径必须能被重写（见 §3.1），否则 `validate` 拦截。
