...
仓内存 Obsidian 风格的 Markdown 笔记（`content/`），由 `scripts/build.mjs` 构建为站点消费的 JSON 产物（`build/`），通过 `raw.githubusercontent.com/GuoxinL/nodes/<branch>/build/...` 直接拉取。

## 写作约定

- 一篇笔记 = `content/` 下一个 `.md` 文件，**文件名（basename）即 slug / 标题**（可用 frontmatter `title` 覆盖）。
