...
> ⚠️ **标题含空格时路径必须包裹尖括号**：`content/<文章>.assets/` 的 `<文章>` 取自文件名，若标题含空格（如 `Markdown 全功能示例`），裸写 `![alt](Markdown 全功能示例.assets/x.webp)` 会被 Markdown 解析器当成纯文本、**不显示**。正确写法二选一：① 尖括号包裹 `![alt](<Markdown 全功能示例.assets/x.webp>)`（推荐）；② 空格编码 `%20`：`Markdown%20全功能示例.assets/x.webp`。构建脚本对两类都按 `raw.githubusercontent.com/<repo>/main/...` 重写。
